...
function isStaticRoute(path: string): boolean {
  ...
  const resolveFullPath = (routePath: string, parentPath: string): string => {
    if (!routePath) return normalizePath(parentPath)
    if (routePath.startsWith('/')) return normalizePath(routePath)
    return normalizePath(`${normalizePath(parentPath)}/${routePath}`.replace(/\/+/g, '/'))
  }

  const checkRoute = (routes: any[], targetPath: string, parentPath = ''): boolean => {
    return routes.some((route) => {
      const routePath = resolveFullPath(String(route.path || ''), parentPath)
  ...
  return checkRoute(staticRoutes, normalizePath(path))
}
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    const mergedMenuList = menuProcessor.mergeMenuLists(menuProcessor.getPublicMenuList(), menuList)

    // 3. 验证菜单数据
    if (!menuProcessor.validateMenuList(mergedMenuList)) {
